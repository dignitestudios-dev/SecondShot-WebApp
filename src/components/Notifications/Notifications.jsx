...
const notifications = [
  {
    ...
    icon:<img src={Alertnoti} alt="Subscription Expiry" className="h-[50px] w-[50px]" />,
    ...
    <div className="absolute -right-[100px]   mt-9 w-[416px] bg-white rounded-3xl shadow-lg z-50">
      ...
      <div className="divide-y divide-gray-200">
        {notifications.map((notif, index) => (
          <div key={index} className={`flex items-start px-5 py-3 ${index === 0 ? 'bg-green-50' : ''}`}>
            <div className={`flex items-center justify-center  rounded-full ${notif.iconBg} ${notif.iconColor}`}>
              {notif.icon}
            </div>
            <div className="ml-4 flex-1">
              <div className="flex justify-between mb-1">
                <div className="font-[500] leading-[21.6px] text-[#303030] text-left text-[14px]">{notif.type}</div>
                <div className="text-[12px] text-[#181818] font-[500] leading-[16.2px] text-left">{notif.date}</div>
              </div>
              <div className="text-[14px] text-[#303030] font-[400] leading-[18.9px] text-left">{notif.message}</div>
            </div>
            {notif.dotColor && (
              <div className="ml-4 mt-1">
                <span className={`h-2 w-2 rounded-full inline-block text-right ${notif.dotColor}`}></span>
              </div>
            )}
          </div>
        ))}
